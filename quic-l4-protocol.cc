          ...
          if (item->m_quicSocket == socket)
            {
              ...
              item->m_udpSocketList.insert(item->m_udpSocketList.end(),udpSocket);  //ywj: replace m_budpSocket with m_udpSocketList to 
              break;                                                                //combine one m_quicSocket with multiple m_udpSocket
            }
        }
  ...
  for (it = m_quicUdpBindingList.begin (); it != m_quicUdpBindingList.end (); ++it)
    {
      ...
      if (item->m_quicSocket == quicSocket)
        {
          return item->m_udpSocketList.back()->GetTxAvailable ();
        }
    }
  return 0;
}
  ...
  for (it = m_quicUdpBindingList.begin (); it != m_quicUdpBindingList.end (); ++it)
    {
      ...
      if (item->m_quicSocket == quicSocket)
        {
          return item->m_udpSocketList.back()->GetRxAvailable ();
        }
    }
  return 0;
}
  ...
  for (it = m_quicUdpBindingList.begin (); it != m_quicUdpBindingList.end (); ++it)
    {
      ...
      if (item->m_quicSocket == quicSocket)
        {
          return item->m_udpSocketList.back()->GetSockName (address);
        }
    }
  ...
  for (it = m_quicUdpBindingList.begin (); it != m_quicUdpBindingList.end (); ++it)
    {
      ...
      if (item->m_quicSocket == quicSocket)
        {
          return item->m_udpSocketList.back()->GetPeerName (address);
        }
    }
  ...
  for (it = m_quicUdpBindingList.begin (); it != m_quicUdpBindingList.end (); ++it)
    {
      ...
      if (item->m_quicSocket == quicSocket)
        {
          item->m_udpSocketList.back()->BindToNetDevice (netdevice);
        }
    }
} 
...
QuicL4Protocol::ForwardUp (Ptr<Socket> sock)
{
  ...
  while ((packet = sock->RecvFrom (from)))
          ...
          if (item->m_quicSocket->GetConnectionId () == connectionId)
            {
              socket = item->m_quicSocket;
                std::cout<<"****ForwardUp(): find socket!!!"<<std::endl;
              break;
            }
        }
   ...
   //ywj: find the subsockId according to srcAddr
      Ipv4Address src = InetSocketAddress::ConvertFrom (from).GetIpv4 ();
std::cout<<"quicL4Protocol.cc forwardup(): src: "<<src<<std::endl;
    uint8_t sFlowIdx = socket->LookUpByAddr(from);
std::cout<<"quicL4Protocol.cc forwardup(): src: "<<src<<"sFlowIdx"<<sFlowIdx<<std::endl;
...
QuicL4Protocol::SetRecvCallback (Callback<void, Ptr<Packet>, const QuicHeader&,  Address& > handler, Ptr<Socket> sock)
{
  ...
  for (it = m_quicUdpBindingList.begin (); it != m_quicUdpBindingList.end (); ++it)
    {
      Ptr<QuicUdpBinding> item = *it;
      if (item->m_quicSocket == sock && item->m_udpSocketList.size() != 0)
        {
...
QuicL4Protocol::CloneSocket (Ptr<QuicSocketBase> oldsock)
{
  ...
  udpBinding->m_udpSocketList.back() = nullptr;
  ...
  while (not found)
    {
      ...
      for (it = m_quicUdpBindingList.begin (); it != m_quicUdpBindingList.end (); ++it)
        {std::cout<<"debug point"<<std::endl;
          found = false;
          Ptr<QuicUdpBinding> item = *it;
          if (connectionId == item->m_quicSocket ->GetConnectionId ())
            {
              break;
            }
          found = true;
...
QuicL4Protocol::SendPacket (Ptr<QuicSocketBase> socket, Ptr<Packet> pkt, const QuicHeader &outgoing) const
{
  ...
  for (it = m_quicUdpBindingList.begin (); it != m_quicUdpBindingList.end (); ++it)
  {
    ...
    if (item->m_quicSocket == socket)
      {
        UdpSend (item->m_udpSocketList[0], packetSent, 0);
        break;
      }
  }
}
